...
def getSchedule(stuId, year, term):
    data = {
        ...
        "_ctl11:ddlSterm": "{}/{}/1 0:00:00".format(year+term-1, 15-term*6),
        "_ctl11:btnSearch": "确定"
    }
    url = "http://jwc.jxnu.edu.cn/MyControl/All_Display.aspx?UserControl=Xfz_Kcb.ascx&UserType=Student&UserNum={}".format(stuId)
    # print(url)
    ...
    try:
        ...
        for tr in trs:
            ...
            #         .format(lesson["course_class_id"], lesson["course_id"], year+term-1, 15-term*6)
        ...
        # 写入数据库
        writeToDB(json.dumps(Schedule, ensure_ascii=False), stuId, year, 'a' if term == 1 else 'b')

        print(Schedule)
        return Schedule

    except:
        print('{}该学期无课程'.format(stuId))


def writeToDB(jsonInfo, stuId, year, term):
    conn, cur = conn_db('localhost', 'root', '', 'api')
    sql = 'INSERT INTO `jxnu_timetable` (`uid`, `year`, `term`, `timetable`) VALUES(\'{}\',{},\'{}\',\'{}\')'.format(stuId, year, term, jsonInfo)
    cur.execute(sql)
    exe_commit(cur)
    conn_close(conn, cur)
...
if __name__ == '__main__':
    # getSchedule('201626703011', 2019, 1)

    conn, cur = conn_db('localhost', 'root', '', 'api')
    cur.execute('select id from `jxnu_students`')
